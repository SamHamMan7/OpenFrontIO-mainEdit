...
export class UnitImpl implements Unit {
  ...
  constructor(
    ...
    this._maxHealth = toInt(this.mg.unitInfo(_type).maxHealth ?? 1);
    if (this._type === UnitType.TransportShip) {
      const extraArmor = "extraArmor" in params ? (params.extraArmor ?? 0) : 0;
      if (extraArmor > 0) {
        this._maxHealth = BigInt(1 + extraArmor * 250);
      }
    }
