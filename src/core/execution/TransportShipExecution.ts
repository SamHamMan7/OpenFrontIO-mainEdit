...
export class TransportShipExecution implements Execution {
  ...
  init(mg: Game, ticks: number) {
    ...
    if (this.extraArmor > 0) {
      const multiplier = Math.floor(this.mg.config().goldMultiplier());
      const extraCost = BigInt(this.extraArmor * 100_000 * multiplier);
      if (this.attacker.gold() < extraCost && !this.mg.config().infiniteGold()) {
        mg.displayMessage(
          "events_display.not_enough_gold",
          MessageType.ATTACK_FAILED,
          this.attacker.id(),
          undefined,
        );
      ...
      if (!this.mg.config().infiniteGold()) {
        this.attacker.removeGold(extraCost);
      }
    }
