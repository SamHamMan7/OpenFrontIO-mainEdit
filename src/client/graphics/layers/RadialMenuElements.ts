...
export const boatMenuElement: MenuElement = {
  ...
  subMenu: (params: MenuElementParams) => {
    ...
    const config = params.game.config();
    const multiplier = config.goldMultiplier();

    for (let extraArmor = 0; extraArmor <= 3; extraArmor++) {
      const extraCost = extraArmor * 100_000 * multiplier;
      const displayCost = extraCost > 0 ? `${renderNumber(extraCost)} ${translateText("player_panel.gold")}` : "Free";
      const hasGold = params.myPlayer.gold() >= BigInt(extraCost) || config.infiniteGold();
      options.push({
        id: `boat_armor_${extraArmor}`,
        name: extraArmor === 0 ? "Standard" : `+${extraArmor} Armor`,
        disabled: () => !hasGold,
        color: hasGold ? COLORS.boat : undefined,
        icon: boatIcon,
        tooltipItems: [
          { text: extraArmor === 0 ? "Standard Transport" : `Armored Transport (+${extraArmor} Hits)`, className: "title" },
          { text: extraArmor === 0 ? "Standard 1-hit transport ship." : `Transport ship with ${extraArmor} extra armor plating.`, className: "description" },
          { text: displayCost, className: "cost" },
        ],
        action: (p: MenuElementParams) => {
          if (hasGold) {
            p.playerActionHandler.handleBoatAttack(p.myPlayer, p.tile, extraArmor);
          }
          p.closeMenu();
        }
      });
